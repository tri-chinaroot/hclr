...
HCLR需要记录介入改变了多少成果，以及改变是否触及任务中的重要部分。本文采用五级量表：
...
**自动化默认口径**：在系统自动记录的场景下，介入量I默认以介入文本的Token数（或字符数）计算。选择Token作默认口径的理由：客观、可自动统计、可复现，使用者零标注负担。边界：Token数测量的是表达长度，不包含形成判断所需的审阅、搜索、核查与推理成本，也不等于独立判断数量；因此Token口径的HCLR反映的是"显性介入表达杠杆"，而非完整的认知投入。不同模型的分词器不同，跨模型比较时需在记录中声明分词口径。
...
第一轮确认衡量使用者通过单位介入产生了多少本人愿意采用的成果改变。对第 \(j\) 次介入：
...
对使用者 \(u\) 在同一模型、同类任务和观察周期内的 \(n\) 次介入，第一轮累计HCLR为：
...
不建议计算单次HCLR的简单算术平均数，因为极短介入可能产生异常高值。累计有效改变除以累计介入量更稳定。报告第二轮HCLR时，还必须同时报告已完成第二轮确认的样本数和待确认比例。

### 4.6 个人变化曲线

HCLR的主要用途是个人纵向自我评估，而不是跨人绝对排名。按周、月、季度或固定任务批次计算：
...
使用者既介入、评分，又完成两轮确认，可能高估自身贡献。该限制来自方法对低成本和可持续性的选择。HCLR明确把结果称为“自我参考评估值”，而不是独立认证结果。
