...
> **文档声明**：本文是HCLR方法的原始方法稿（v0.1），用于传播、引用与社区讨论，非正式学术期刊发表。文中的公式与参考值均为**实验性设计**：P1–P5为顺序等级，尚未经过量表标定；介入量口径需在记录时声明。读者不应将任何数值解读为已验证的测量结果。方法的最新、最简说明见本仓库[README](README_CN.md)。
>
> 版本：0.1（方法稿，第二版原始稿）
>
> 作者：tri-chinaroot
...
其中，独立语义命题数量最接近“信息量”。例如，“数据口径不一致，因此市场规模不能直接比较”包含事实判断和推论判断，可以记为两个相关命题。为了降低标注成本，实际应用也可以先用字数或Token数作为近似值。
