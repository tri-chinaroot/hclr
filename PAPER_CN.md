...
> 版本：0.3（方法稿，第三版原始稿）
>
> 修订历史：v0.1（2026-08-12）初始方法稿；v0.2（2026-08-12）介入量Token默认口径、分析单位统一为任务事件、新增流程图与术语表、补充测量学文献；v0.3（2026-08-12）核心公式改为输出/介入杠杆率（O/I），P与双重确认降为辅助指标，消除序数量表直接参与计算的测量学问题。
>
> 作者：tri-chinaroot
...
HCLR评估的不是人的一般智力，也不判断AIGC成果是否达到抽象意义上的客观真理。它关注一个更具体的实践问题：在给定模型、任务领域和相对稳定的使用周期内，使用者以多少显性介入（Token计），使模型产生多少输出（Token计）。核心比率定义为**输出/介入杠杆率**：模型输出Token总和除以使用者介入Token总和。双重确认（使用者确认采纳、根据受众反馈确认认可）与改变范围P1–P5不再进入公式，作为辅助指标单独报告。
...
> 在给定大模型、任务领域和观察周期内，AI使用者以有限的显性介入（Token计），使模型产生的输出规模（Token计）发生可观察的杠杆效应。核心指标为输出/介入杠杆率；采纳确认、受众认可确认与改变范围作为辅助指标单独报告。
...
双重确认在HCLR中作为辅助指标使用，不进入输出/介入杠杆率的计算；它回答的是"改变是否被采纳、是否获受众认可"的质量问题。
...
以下指标回答杠杆率之外的质量问题，单独报告：

| 指标 | 定义 | 说明 |
|---|---|---|
| 介入后采纳率 | C1=1（含部分采纳）任务数 / 全部任务数 | 改变是否被使用者采用 |
| 第二轮完成率 | 有C2结果任务 / C1=1任务 | 反馈回填完整度 |
| 已确认认可率 | C2=1 / 有C2结果任务 | 已确认样本中的认可比例 |
| 待确认比例 | S1任务数 / 全部任务数 | 待确认不按失败处理 |
| P1–P5分布 | 改变范围的描述性统计 | 介入性质 |
...
该数值没有跨领域的天然含义，只有在相同评分规则、相近任务和相同模型环境中比较才有意义。示例中，改变范围评为P4（方案改变）、第一轮确认C1=1（采纳）——这些作为辅助指标与杠杆率一并报告。
## 5. 轻量测量协议
...
改变范围P1–P5不再进入杠杆率计算，单纯追求高P值不会直接抬高分数；但P分布仍反映介入性质，应结合采纳确认与认可确认阅读。

### 9.2 使用者承担全部确认

使用者既介入、评分，又完成辅助确认，可能高估自身贡献[9]。该限制来自方法对低成本和可持续性的选择。核心杠杆率完全由Token统计得到，不依赖使用者的评分判断；辅助指标则明确称为“自我参考评估值”。
...
使用者按固定周期持续记录，可以形成个人变化曲线，观察自己是否能以更少的介入Token产生更多的模型输出Token。双重确认与改变范围作为辅助指标，回答杠杆率之外的质量问题。曲线是AIGC使用情境下输出/介入杠杆的参考，不是人的一般认知水平或永久能力分数。
