...
> **文档声明**：本文是HCLR方法的原始方法稿（v0.1），用于传播、引用与社区讨论，非正式学术期刊发表。文中的公式与参考值均为**实验性设计**：P1–P5为顺序等级，尚未经过量表标定；介入量I在自动记录场景下默认以Token数（或字符数）计算，测量的是显性介入表达杠杆，不包含形成判断所需的认知投入成本。读者不应将任何数值解读为已验证的测量结果。方法的最新、最简说明见本仓库[README](README_CN.md)。
>
> 版本：0.2（方法稿，第二版原始稿）
>
> 修订历史：v0.1（2026-08-12）初始方法稿；v0.2（2026-08-12）介入量Token默认口径、分析单位统一为任务事件、新增流程图与术语表、补充测量学文献。
>
> 作者：tri-chinaroot
...
HCLR评估的不是人的一般智力，也不判断AIGC成果是否达到抽象意义上的客观真理。它关注一个更具体的实践问题：在给定模型、任务领域和相对稳定的使用周期内，使用者以多少显性介入，使AIGC成果发生多大程度的有效改变。本文采用由使用者完成的双重确认：成果生成后，使用者确认本人是否采纳；成果触达目标受众后，使用者再根据真实反馈确认受众是否认可。第一轮形成即时参考值，第二轮形成结果确认值。尚未获得受众反馈的记录保持待确认状态，不按失败处理。

本文给出HCLR的定义、计算方式、五级改变范围、介入量记录方法以及个人层面的汇总方法，并提出一套轻量测量协议。使用者持续完成两轮确认后，可以形成个人HCLR变化曲线，观察自己能否以更少、更关键的判断产生更多可采纳成果。该数据还可为模型厂商补充现有即时好坏反馈：第一轮记录成果是否真正被采用，第二轮记录采用后是否得到受众认可；如果同时保存原始输出、稀疏介入和修改后成果，重复出现的高杠杆介入可以转化为模型评测项和训练材料。本文不把HCLR主张为AI使用能力的唯一指标，也不把它作为脱离模型和任务环境的永久能力分数。
...
> 在给定大模型、任务领域和观察周期内，AI使用者以有限的显性介入，使AIGC成果先被本人采纳，并在成果触达受众后由使用者根据反馈确认获得认可的程度。
...
“稀疏”应由介入量与改变范围的关系事后判断，而不是提前限制使用者只能提出几个问题。使用者可以完整阅读和自由介入；如果最终只有少数判断产生主要改变，就形成了可观察的稀疏介入现象。
...
A_j=
\begin{cases}
1,& \text{第 }j\text{ 次介入后的成果被使用者采纳}\\
0,& \text{未被采纳}
\end{cases}
$$

> 注：早期草稿中的采纳变量 $A_j$ 与 3.5 节第一轮确认 $C_{1j}$ 含义相同。正式记录与计算统一使用 $C_{1j}$，$A_j$ 不再单独使用。

也可以采用0至1的分级值，表示局部采用、修改后采用和直接采用。但在方法初期，二元记录更容易形成一致标准。
...
HCLR需要记录介入改变了多少成果，以及改变是否触及任务中的重要部分。该量表为顺序等级（ordinal），等级间距未经等距性标定[8]。本文采用五级量表：

| 等级 | 改变范围 | 判断示例 |
|---|---|---|
| P1 | 局部改变 | 修正一处事实、措辞或单项内容，不影响主要结论 |
| P2 | 模块改变 | 改变一个主要段落、功能模块或一条完整论证 |
| P3 | 结论改变 | 改变一个核心结论，或联动修改多个主要部分 |
| P4 | 方案改变 | 改变主要建议、方案排序、交付结构或行动方向 |
| P5 | 框架改变 | 改变问题定义、分析框架或整份成果的组织方式 |
...
改变范围不等于改写字数。删除一句错误的核心假设可能改变整项结论；重写数百字措辞则可能仍属于P1。评分应依据最终成果在任务中的作用，而不是文本差异大小。
...
**自动化默认口径**：在系统自动记录的场景下，介入量I默认以介入文本的Token数（或字符数）计算。选择Token作默认口径的理由：客观、可自动统计、可复现，使用者零标注负担。边界：Token数测量的是表达长度，不包含形成判断所需的审阅、搜索、核查与推理成本[10]，也不等于独立判断数量；因此Token口径的HCLR反映的是"显性介入表达杠杆"，而非完整的认知投入。不同模型的分词器不同，跨模型比较时需在记录中声明分词口径。
...
> **实验性公式**：P1–P5为顺序等级，未标定前不宜作为等距数值直接参与计算；I_j的取值口径（判断数/字数/次数）需在记录时声明。正式使用请以仓库README中的口径说明为准。

第一轮确认衡量使用者通过单位介入产生了多少本人愿意采用的成果改变。对第 $j$ 个任务事件：
...
其中，$C_{1j}$为使用者是否采纳，$P_j$为改变范围，$I_j$为介入信息量。如果使用者不采纳，第一次确认不通过，该次介入不产生采纳杠杆。

### 4.2 第二轮：认可HCLR

> **实验性公式**：第二轮仅在已获得受众反馈的记录上计算；待确认（S1）不计入0。分母口径与4.1相同。

成果触达受众后，使用者根据实际反馈完成第二轮确认：
...
第二轮确认只适用于已经获得受众反馈的记录。如果使用者确认受众认可，$C_{2j}=1$；如果确认未获认可，$C_{2j}=0$。两轮确认均由使用者完成，区别在于第一轮依据本人采用决定，第二轮依据成果进入真实受众场景后的反馈。
...
> **口径说明**：自动记录场景下，I_semantic默认以介入Token数（或字符数）计算；人工记录场景下可使用独立判断数或语义命题数。同一曲线内不得混用两种口径。

介入信息量和介入时间回答不同问题，不建议强行合成一个分母。第一轮和第二轮都可以分别计算信息杠杆率与时间杠杆率。
...
时间杠杆率以介入时间 $T_j$ 替换分母。信息杠杆率回答“单位显性判断改变了多少可采纳成果”，时间杠杆率回答“单位时间产生了多少可采纳改变”。

### 4.5 个人层面的汇总

对使用者 $u$ 在同一模型、同类任务和观察周期内的 $n$ 个任务事件，第一轮累计HCLR为：
...
第二轮累计HCLR只在已经完成第二轮确认的记录集合 $J_2$ 上计算：
...
不建议计算单次任务事件HCLR的简单算术平均数，因为极短介入可能产生异常高值。累计有效改变除以累计介入量更稳定。报告第二轮HCLR时，还必须同时报告已完成第二轮确认的样本数和待确认比例。

### 4.6 个人变化曲线

HCLR的主要用途是个人纵向自我评估[11]，而不是跨人绝对排名。按周、月、季度或固定任务批次计算：
...
两条曲线分别表示使用者把AIGC成果转化为“本人愿意采用”和“经受众反馈确认获得认可”的效率变化。还可以报告第二轮确认率：
...
曲线上升可以作为稀疏介入能力提升的参考，但不能脱离环境解释。模型版本、任务领域、任务难度、目标受众和介入量单位应保持相对稳定。若这些条件变化，应开始新的观察区间或在曲线上明确标记。

HCLR变化曲线不证明人的一般认知水平提高了。它反映的是：在特定AIGC使用环境中，使用者是否越来越能以更少、更关键的介入产生更多可采纳成果。

### 4.7 一个简化示例

> **单位口径说明**：本示例以"任务事件"为单位——3个判断共同促成一次P4改变，按总介入量计算一次参考值。单条介入不应重复计算同一成果改变；应用时需固定单位口径（见README SCORING口径）。

某使用者审阅一份AI生成的市场进入报告，提出三个独立判断：

1. “样本只覆盖一线城市，不能代表全国需求。”
2. “竞争对手不会保持价格不变。”
3. “缺少渠道建设成本，当前利润预测不能支持进入结论。”

三项判断合计记为3个介入语义命题。AI据此把主要建议从“立即全国进入”改为“先在两个区域试点并补充渠道成本验证”。使用者采用新版本，改变范围评为P4。
...
成果提交投资委员会后，如果使用者根据会议结果确认方案进入试点阶段，则第二轮状态为S3，并形成相应的 $HCLR_I^{(2)}$。如果会议尚未召开，则状态为S1，而不是S2。这个数值没有跨领域的天然含义，只有在相同评分规则、相近任务和相同模型环境中比较才有意义。
...
HCLR是一种自我评估方法，使用者既是介入者，也是两轮确认和改变范围的主要记录者。这一设计不是为了得到脱离情境的客观评分，而是为了让使用者以较低成本持续观察自己的AI应用表现。

为降低随意性，系统可以提供统一定义和实例，要求使用者在不知道当期HCLR结果前完成P1至P5评分。研究或组织应用中可以抽取少量样本交由另一名评分者复核，但独立评分不是每次记录的必要环节。
...
让使用者在不同时间对同一批匿名案例进行P1至P5评分，检查其判断是否大致稳定。研究场景中还可以由另一名评分者复核少量样本，识别量表中容易混淆的边界。若P3与P4长期难以区分，应调整定义和示例。
...
如果只追求P值，使用者可能故意提出颠覆性意见，使AI大幅重写。HCLR通过两轮确认约束这种行为。没有进入实际采用的改变不能形成第一轮分数，未获受众认可的成果不能形成第二轮价值。

### 9.2 使用者承担全部确认

使用者既介入、评分，又完成两轮确认，可能高估自身贡献[9]。该限制来自方法对低成本和可持续性的选择。HCLR明确把结果称为“自我参考评估值”，而不是独立认证结果。
...
战略方案、营销文案和代码评审的改变范围不可直接互换。HCLR适合在同领域、同任务族中使用。跨领域汇总只能作为个人档案描述，不能作为精确排名。

### 9.7 指标博弈

如果HCLR直接绑定绩效、薪酬或淘汰，使用者可能拆分介入、夸大改变范围或选择容易产生高分的任务。HCLR在早期更适合：
...
使用者按固定周期持续记录两个值，可以形成个人变化曲线，观察自己是否能以更少、更关键的介入产生更多可采纳成果。曲线是AIGC使用情境下稀疏介入能力的参考，不是人的一般认知水平或永久能力分数。

同一双重确认数据还可能帮助模型厂商提高生成质量。与即时好坏评价相比，它进一步记录成果是否真正被采用，以及采用后是否经受众反馈确认。如果同时保留原始输出、稀疏介入和修改后成果，高频高杠杆介入可以转化为模型评测项和训练材料。
