...
> **文档声明**：本文是HCLR方法的原始方法稿（v0.1），用于传播、引用与社区讨论，非正式学术期刊发表。文中的公式与参考值均为**实验性设计**：介入量I在自动记录场景下默认以Token数（或字符数）计算，测量的是显性介入表达杠杆，不包含形成判断所需的认知投入成本。读者不应将任何数值解读为已验证的测量结果。方法的最新、最简说明见本仓库[README](README_CN.md)。
>
> 版本：0.4（方法稿，第四版原始稿）
>
