...
[TOA询问法](https://github.com/tri-chinaroot/toa-questioning) 是与HCLR**相互独立**的方法，解决不同问题：

| | TOA | HCLR |
|---|---|---|
| 目的 | 获取使用者的意图/知识 | 评估使用者对模型输出的影响 |
| 产出 | Q&A文档（信息记录） | 杠杆率数值（测量结果） |
| 行为 | AI提问、使用者回答（提供信息） | 使用者介入、修改模型已有输出 |
| 主动方 | AI主动提问 | 使用者主动介入 |

**注意**：TOA流程中的使用者回答**不是**HCLR意义上的"介入"——回答问题是提供信息，不是修改已有输出。因此TOA记录**不直接作为**HCLR任务事件。两者可在同一工作流中先后出现（如先TOA获取需求，再生成初稿并按HCLR测量后续介入），但这是两个独立方法的先后使用，不是数据依赖关系。
