...
[TOA询问法](https://github.com/tri-chinaroot/toa-questioning) 是与HCLR**相互独立**的方法：TOA通过AI逐题提问获取使用者的意图与知识（产出Q&A文档），HCLR评估使用者对模型输出的介入影响（产出杠杆率）。两者解决不同问题，可独立使用；也可在同一工作流中先后出现，但无数据依赖关系。
